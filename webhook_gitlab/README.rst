...
**Create Webhooks** button on the Odoo project form registers a hook on
the repository (push, merge request and pipeline events, secured with
``webhook_gitlab.authorization_token``). Deploying again replaces the
hook, so the button can be reused after changing the Odoo base URL or
the token.
