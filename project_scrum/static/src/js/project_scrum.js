odoo.define('project_scrum.update_kanban', function (require) {
'use strict';
...
KanbanRecord.include({
    on_card_clicked: function () {
        if (this.model === 'project.scrum.us') {
    ...
    on_kanban_action_clicked: function (ev) {
        var self = this;
        if (this.model === 'project.scrum.test' && $(ev.currentTarget).data('type') === 'set_cover') {
            ev.preventDefault();

            new Model('ir.attachment').query(['id', 'name'])
               .filter([['res_model', '=', 'project.scrum.test'], ['res_id', '=', this.id], ['mimetype', 'ilike', 'image']])
        ...
        function open_cover_images_dialog(attachment_ids) {
            ...
            var $content = $(QWeb.render("project_scrum.SetCoverModal", {
                cover_id: cover_id,
