...
class ProfitabilityAnalysis(models.Model):
    ...
    consumable_cost = fields.Float(
        "Consumable Cost", digits=(16, 2), readonly=True, group_operator="sum"
    )

    def init(self):
        """Overwrite to add consumable_cost field and related impacted

        allow larger code to make easy diff with odoo source code
        """
        ...
        query = (
            ...
            CREATE VIEW %s AS (
                SELECT
                    ...
                    sub.consumable_cost as consumable_cost,
                    ...
                    (sub.expense_amount_untaxed_to_invoice + sub.expense_amount_untaxed_invoiced + sub.amount_untaxed_to_invoice +
                        sub.amount_untaxed_invoiced + sub.other_revenues + sub.timesheet_cost + sub.expense_cost + sub.consumable_cost)
                        as margin
                FROM (
                    SELECT
                        ...
                        COST_SUMMARY.consumable_cost AS consumable_cost,
                    ...
                    FROM project_project P
                        ...
                        LEFT JOIN (
                            ...
                            SELECT
                                ...
                                SUM(consumable_cost) AS consumable_cost,
                            ...
                            FROM (
                                ...
                                FROM account_analytic_line TS
                                    INNER JOIN project_project P ON TS.project_id IS NOT NULL AND P.id = TS.project_id
                                WHERE
                                    P.active = 't'
                                ...
                                SELECT
                                    Pcons.id AS project_id,
                                    Pcons.analytic_account_id AS analytic_account_id,
                                    TScons.so_line AS sale_line_id,
                                    0.0 AS timesheet_unit_amount,
                                    TScons.amount AS consumable_cost,
                                    ...
                                    0.0 AS amount_untaxed_to_invoice,
                                    0.0 AS amount_untaxed_invoiced,
                                    TScons.date AS line_date
                                FROM account_analytic_line TScons
                                    INNER JOIN project_project Pcons ON TScons.project_id IS NOT NULL AND Pcons.id = TScons.project_id
                                WHERE
                                    Pcons.active = 't'
                                    AND TScons.product_id IS NOT NULL
