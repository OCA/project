...
class TestTask(TransactionCase):
    ...
    def test_task_statistics(self):
        ...
        self.assertFalse(t1.date_opened, 'New task cannot have open date')
        self.assertFalse(t1.date_closed, 'New task cannot have close date')
        # Done task
        t1.stage_id = stage_done
        self.assertIsNot(
            t1.date_opened, False, 'Done task must have open date')
        self.assertIsNot(
            t1.date_closed, False, 'Done task must have close date')
        # (Re)Open task
        t1.stage_id = stage_open
        self.assertIsNot(
            t1.date_opened, False, 'Open task must have open date')
        self.assertFalse(t1.date_closed, 'Open task cannot have close date')
