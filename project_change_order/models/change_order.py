...
class ChangeOrder(models.Model):
    ...
    def _default_stage_id(self):
        return self.env.ref('project_change_order.change_order_stage_draft')

    @api.one
    def action_review(self):
        return self.write({'stage_id': self.env.ref(
                'project_change_order.change_order_stage_review').id})

    @api.one
    def action_draft(self):
        return self.write({'stage_id': self.env.ref(
                'project_change_order.change_order_stage_draft').id})

    @api.one
    def action_approve(self):
        if self.change_order_line_ids:
            for change_line in self.change_order_line_ids:
                change_line.budget_line_id.planned_amount += change_line.change_value
        return self.write({'stage_id': self.env.ref(
                'project_change_order.change_order_stage_approved').id})
    ...
    def action_cancel(self):
        if self.change_order_line_ids:
            for change_line in self.change_order_line_ids:
                change_line.budget_line_id.planned_amount -= change_line.change_value
        return self.write({'stage_id': self.env.ref(
                'project_change_order.change_order_stage_canceled').id})
