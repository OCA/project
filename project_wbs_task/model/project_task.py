...
class task(orm.Model):
    ...
    def _project_complete_wbs_name(
        ...
        for task in tasks:
            if task.project_id:
                ...
                data_project = project_obj.read(
                    cr, uid, task_project_id, ['complete_wbs_name'], context=context
                )
            if data_project:
                res.append(
                    (task.id, data_project['complete_wbs_name'])
                )
            else:
                res.append(
                    (task.id, '')
    ...
    def _project_complete_wbs_code(
        self, cr, uid, ids, prop, unknow_none, context=None
    ):
        ...
        for task in tasks:
            if task.project_id:
                task_project_id = task.project_id.id
                data_project = project_obj.read(
                    cr, uid, task_project_id, ['complete_wbs_code'], context=context
                )
            if data_project:
                res.append(
                    (task.id, data_project['complete_wbs_code'])
                )
            else:
                res.append(
                    (task.id, '')
