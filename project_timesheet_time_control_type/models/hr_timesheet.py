...
class AccountAnalyticLine(models.Model):
        ...
        for line in self.filtered(lambda line: not line.time_type_id):
            ...
            if not employee_id:
                ...
                if employee_id:
                    ...
                    employee = self.env['hr.employee'].search([('user_id', '=', self.env.user.id), ('company_id', '=', self.env.company.id)], limit=1)
                if employee:
    ...
    def _onchange_project_type_id(self):
        if self.time_type_id and (not self.name or self.name == "/"):
            self.name = "{}: ".format(self.time_type_id.code) if self.time_type_id.code else self.time_type_id.name
