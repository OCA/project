...
- The CI status of a pull request (``ci_status`` field and the
  ``CI: ...`` task tags) is never populated on GitHub: unlike the GitLab
  bridge, which tracks pipeline events, no CI event handler is
  implemented yet. GitHub reports CI through several webhook families
