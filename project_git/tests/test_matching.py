...
from .common import GITHUB_REPO_URL, GITLAB_REPO_URL, ProjectGitCase


class TestTaskMatching(ProjectGitCase):
    ...
    def test_matching_runs_as_the_public_user(self):
        ...
        # mechanisms must work through their targeted sudo
        public_git_event = self.git_event.with_user(self.env.ref("base.public_user"))
        matching_tasks = public_git_event._find_matching_tasks(
            projects=self.gitlab_project,
            pattern_text=f"GL-100 fix tid#{self.gl_task_115.id}",
        )
        self.assertEqual(matching_tasks, self.gl_task_100 | self.gl_task_115)
