...
class TestProjectTaskGitSmartButtons(ProjectGitCase):
    ...
    def setUpClass(cls):
        ...
        cls.git_branch = cls.env["project.git.branch"].create(
            {"name": "GL-100-feature", "task_ids": task_link}
        )
        cls.git_commits = cls.env["project.git.commit"].create(
            [
                {
        ...
        cls.git_pull_request = cls.env["project.git.pull.request"].create(
            {"name": "GL-100 feature", "task_ids": task_link}
        )
