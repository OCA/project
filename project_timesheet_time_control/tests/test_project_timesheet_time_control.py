...
class TestProjectTimesheetTimeControl(common.TransactionCase):
    def setUp(self):
        super().setUp()
        admin = self.browse_ref("base.user_root").with_context(
    ...
    def test_aal_time_control_flow(self):
        ...
        self.assertLessEqual(wizard.date, fields.Date.today())
        ...
        wizard.date_time = fields.Datetime.to_string(
            fields.Datetime.from_string(running_timer.date_time) +
        ...
        self.assertEqual(
            fields.Datetime.from_string(new_line.date_time),
    ...
    def test_wizard_standalone(self):
        ...
        new_line.with_context(
            # Stop the line in one minute in future, otherwise line will not be
            # stopped, because there is no time difference between start and
            # end dates
            stop_dt=fields.Datetime.to_string(
                datetime.now() + timedelta(minutes=1))
        ).button_end_work()
