...
class analytic_account_sequence(osv.osv):
    ...
    _columns = {
        ...
        'number_next_actual': fields.function(
            ...
            string='Next Number',
            help='''
            Next number that will be used. This number can be incremented frequently so the displayed
            value might already be obsolete
            '''
        ),
        ...
        'padding': fields.integer(
            ...
            required=True,
            help="""
            OpenERP will automatically add some '0' on the left of the 'Next Number' to get the required padding size.
            """),
    ...
    _sql_constraints = [('unique_analytic_account_id', 'unique (analytic_account_id)',
    ...
    def _create_sequence(self, cr, id, number_increment, number_next):
        ...
        sql = "CREATE SEQUENCE analytic_account_sequence_%05d INCREMENT BY %%s START WITH %%s" % id
    ...
    def create(self, cr, uid, values, context=None):
        ...
        values['id'] = super(analytic_account_sequence, self).create(cr, uid, values, context)
        if values['implementation'] == 'standard':
            self._create_sequence(cr, values['id'], values['number_increment'], values['number_next'])
        return values['id']

    def unlink(self, cr, uid, ids, context=None):
        super(analytic_account_sequence, self).unlink(cr, uid, ids, context)
    ...
    def write(self, cr, uid, ids, values, context=None):
        ...
        super(analytic_account_sequence, self).write(cr, uid, ids, values, context)
    ...
    def _next(self, cr, uid, seq_ids, context=None):
        ...
        sequences = self.read(cr, uid, seq_ids, ['name', 'company_id',
                                                 'implementation', 'number_next',
                                                 'prefix', 'suffix', 'padding'])
        preferred_sequences = [s for s in sequences if s['company_id'] and s['company_id'][0] == force_company]
        seq = preferred_sequences[0] if preferred_sequences else sequences[0]
        if seq['implementation'] == 'standard':
            cr.execute("SELECT nextval('analytic_account_sequence_%05d')" % seq['id'])
            seq['number_next'] = cr.fetchone()
        else:
            cr.execute(
                "SELECT number_next FROM analytic_account_sequence WHERE id=%s FOR UPDATE NOWAIT", (seq['id'],)
            )
            cr.execute(
                "UPDATE analytic_account_sequence SET number_next=number_next+number_increment WHERE id=%s ",
                (seq['id'],)
            )
    ...
    def next_by_id(self, cr, uid, sequence_id, context=None):
        ...
        ids = self.search(
            cr,
            uid,
            ['&', ('id', '=', sequence_id), ('company_id', 'in', company_ids)]
        )
