...
class TestDuplicatedTasksProcesses(TransactionCase):
    def setUp(self):
        ...
        self.task1.duplicated_main_task_ids = [(4, self.task2.id)]
    ...
    def test_close_duplicated_task(self):
        ...
        self.assertTrue(self.task2.id in self.task1.duplicated_main_task_ids.ids)
        self.task1._fill_duplicated_in_main_task()
        self.assertTrue(self.task1.id in self.task2.duplicated_task_ids.ids)
    ...
    def test_close_duplicated_task_in_cascade(self):
        ...
        self.task2.duplicated_main_task_ids = [(4, self.task3.id)]
        self.task2._fill_duplicated_in_main_task()
        self.assertTrue(self.task1.id in self.task3.duplicated_task_ids.ids)
        self.assertTrue(self.task3.id in self.task1.duplicated_main_task_ids.ids)

    def test_reopen_duplicated_task(self):
        self.task1.duplicated_task_status_action()
        self.assertFalse(self.task1.is_duplicate)
        self.assertEqual(len(self.task1.duplicated_main_task_ids), 0)
        self.task1._fill_duplicated_in_main_task()
        self.assertFalse(self.task2.have_duplicate)
        self.assertEqual(len(self.task2.duplicated_task_ids), 0)
    ...
    def test_duplicate_task_with_subtasks(self):
        ...
        self.subtask1._fill_duplicated_in_main_task()
        self.assertTrue(self.subtask1.id in self.task2.duplicated_task_ids.ids)
        self.assertTrue(self.task2.id in self.subtask1.duplicated_main_task_ids.ids)
