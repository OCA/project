...
class contract_sub_line(osv.osv):
    ...
    def _get_location(self, cr, uid, ids, product_id, arg, context=None):
        ...
        for sub_contract_line_id in ids:
            location_id = False
            prod_lot_id = self.browse(
                cr, uid, sub_contract_line_id, context).stock_production_lot_id
            if prod_lot_id:
                move_ids = stock_obj.search(cr, uid, [(
                    'prodlot_id', '=', prod_lot_id.id), ('state', '=', 'done')], order='location desc', limit=1)
                if move_ids:
                    location_id = stock_obj.browse(
                        cr, uid, move_ids[0], context).location_dest_id.id
