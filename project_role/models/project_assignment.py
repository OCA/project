        ...
        for assignment in self:
            if assignment.project_id:
                assignment.name = _(" %(user_id)r as %(role_id)r on %(project_id)r") % {
                    "user_id": assignment.user_id.name,
                    "role_id": assignment.role_id.name,
                    "project_id": assignment.project_id.name,
                }

            elif assignment.company_id:
                assignment.name = _(" %(user_id)r as %(role_id)r in %(company_id)r") % {
                    "user_id": assignment.user_id.name,
                    "role_id": assignment.role_id.name,
                    "company_id": assignment.company_id.name,
                }
            else:
                assignment.name = _(" %(user_id)r as %(role_id)r") % {
                    "user_id": assignment.user_id.name,
                    "role_id": assignment.role_id.name,
                }
        ...
        for assignment in self:
            ...
            if conflicting_assignment:
                raise ValidationError(
                    _(
                        "Assignment %(name)r conflicts with another assignment: "
                        "%(conflicting_name)r"
                    )
                    % {
                        "name": assignment.name,
                        "conflicting_name": conflicting_assignment.name,
                    }
                )
            if not assignment.role_id.can_assign(
                ...
                if assignment.project_id:
                    error = _(
                        "User %(user_id)r can not be assigned to role %(role_id)r"
                        " on %(project_id)r."
                    ) % {
                        "user_id": assignment.user_id.name,
                        "role_id": assignment.role_id.name,
