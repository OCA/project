...
class CapacityLineReporting(models.TransientModel):
    _name = "capacity.line.reporting"
    _description = "Capacity line reporting wizard"
    ...
    granularity = fields.Selection(
        ...
        default=lambda r: r.env.company.capacity_line_granularity,
        required=True,
    )
    ...
    def _compute_bokeh_chart(self):
        ...
        # employee, the chart displays the consolidated_capacity_hours field
    ...
    def _prepare_bokeh_chart_data(self):
...

        {
            employee: {
                project: {
                    date: capacity
                    for dates with a capacity on that project
                }
        ...
        groupdata = self.env["capacity.line"].read_group(
            domain, ["consolidated_capacity_hours"], groups, lazy=False
        )
        employees = set()
        projects = set()
        data = {}
        for d in groupdata:
            ...
            capacity = d["consolidated_capacity_hours"]
            date = d["__range"]["date_from"]["from"]
            x_key = date
            data[employee][project][x_key] = capacity
    ...
    def _build_plots(self, height=300, width=1024):
        ...
        for employee in employees:
            ...
            for project in data[employee]:
                capacity = data[employee][project]
                plot_data[project] = [capacity.get(date, 0) for date in dates]
